...
%\subsection{FTP を使ってインストール}
%
%ブート USB メモリ (HTTP/NFS/FTP 対応) を用意して, Linux をインストー
%ルしたいマシンを起動してください.

%\begin{enumerate}
%	\item boot プロンプトが出てきたら Enter を押す.
%	\item Japanese を選択する.
%	\item Keyboard Type の選択をする.
%	\item Installation Method の選択をする.
%	\item FTP image の選択をする.
%	\item Configure TCP/IP の設定をする.
%%ここでは, IP Address (133.62.236.xxx), Netmask (255.255.255.224)
%%の 2 つの設定を行えばよい. xxx には自分のマシンの所有する IP アド
%%レスを当てはめて使おう.
%	\item  FTP/NFS Setup の設定をする.
%%FTP server name  に\verb|ftp.info.sci.yamaguchi-u.ac.jp|を,
%%Vine Linux directory に\\\verb|/pub/VineLinux/Vine-3.2/i386|を入力する.
%	\item  あとはインストールディスクを使う場合と同じです.
%\end{enumerate}
...
%\subsection{OpenOffice.org をインストール}
%
%OpenOffice.org は Microsoft Office のファイルの読み込みもできる, オフィスソフトパックです.
%表計算, 文章作成, プレゼン資料作成等いろいろできます.
%以下のようにしてインストーラーを起動し, 画面の指示に従ってインストールしましょう.
%\begin{verbatim}
%# /home/public/openoffice.org/setup
%\end{verbatim}
%\subsection{StarSuite をインストール}
%
%StarSuite は Microsoft Office のファイルの読み込みもできる, オフィスソフトパックです.
%表計算, 文章作成, プレゼン資料作成等いろいろできます.
%以下のようにしてインストールしましょう.
%\begin{verbatim}
%# cd /home/public/unpack_starsuite/RPMS
%# rpm -ivh star*.rpm
%\end{verbatim}
...
%メーラにはいろいろありますが, sylpheed や thunderbird が軽くて人気があり
%ます. sylpheed は WindowMaker のアイコンから起動します.
%thunderbird は firefox (mozilla の軽量版) との兄弟ソフトで,
メーラにはいろいろありますが, thunderbird や sylpheed が人気があります.
%\begin{verbatim}
%# apt-get install thunderbird
%\end{verbatim}
%でインストールできます.
% emacs 上で動くメーラ mew も根強い人気があります. mew の設定は
% \verb|~/etc/emacs/mew.emacs-4.1.el|を編集します.
...
%% また, メールの操作として「削除」は行っても OK ですが, 「フォルダへ移動」と
%% いう項目は (メニューにあっても) 使わないで下さい.
%% メールの整理は学校に来てから mew とか sylpheed などでいつものように行って下さい.
...
%% \section{その他研究室特有の計算機設定}
%%無線 LAN の使い方, MATLAB の使い方は裏頁等, ほかに移動して, このドキュ
%%メントは「はじめてのインストールガイド」と位置づけたい.

%% \subsection{ノート PC の無線 LAN 設定}
%% \subsubsection{ノ-トパソコンで無線 LAN を利用する (検証まだあり)}

%% ノ-トパソコンを持っているときはぜひ活用してください.
%% しかし, まだ未対応のカ-ドもありますので, 注意してください.
%% \begin{enumerate}
%% \item パソコンを起動する.
%% \item 起動したら無線 LAN カ-ドをパソコン横のスロットに差し込む.
%% ぴぴっと音が鳴ったことを確認する.
%% \item ログインして, root になる.
%% \item ちゃんと認識しているか確認する (以下のように製品の会社名などがでたら OK).
%% \begin{verbatim}
%% # cardctl ident
...
%% \end{verbatim}
%% \item 
%% エディタで \verb|/etc/pcmcia/wireless.opts| の一部を変更する. 変更する
%% 箇所は 3 つで, 
%% \verb|ESSID="0007409685DE", KEY="s<パスワード>",MODE="Managed"| 
%% です. 他は以下のようにコメントし, パスワ-ドは研究室のヒトに聞いてください.
%% \begin{verbatim}
...
%%     ;;
%% \end{verbatim}
%% \item \verb|/etc/sysconfig/network-scripts/ifcfg-eth<整数>| を以下のように
%% 作成してください. なお, 整数は iwconfig eth<整数> と同じ
%% ものにしてください.
%% \begin{verbatim}
%% DEVICE=eth&lt;上の整数と同じ数>
%% USERCTL=no
...
%% \item 
%% 以上は何度も設定する必要はありません.
%% \item 無線 LAN の内容を以下のコマンドで設定する.
%% \begin{verbatim}
%% # ./etc/pcmcia/wireless start eth<整数>
%% \end{verbatim}
%% \item 
%% 無線 LAN でネットワ-クに接続する.
%% \begin{verbatim}
...
%% 無線のカ-ドによってはまだまだ未対応のものもありますので,
%% 設定できない場合は誰か (谷合ならいつでもどうぞ) を捕まえて聞いてください.

%% \subsection{Matlab の利用}

%% \subsubsection{Matlab の利用方法}

%% \begin{enumerate}
%% \item 自分のモニタで GUI 操作するために, Matlab サーバからの X の転送を許可します.

%%       \begin{verbatim}
%%       [yosi@(自分のマシンのホスト名) yosi]$ xhost +aris
%%       \end{verbatim}

%% \item 以下のように Matlab サーバにログインする. 

%%       \begin{verbatim}
%%       [yosi@triton yosi]$ ssh -X -l yosi aris
%%       yosi@aris's password: 
%%       Last login: Fri Sep 10 16:22:52 2004 from triton.bcl.sci.yamaguchi-u.ac.jp
%%       [yosi@aris yosi]$ 
%%       \end{verbatim}

%%       -X オプションは X の forward をするためのオプションです (オプションつけなくて
%%       もできるかも)
 
%% \item Matlab のプログラムを実行する.

%%       \begin{verbatim}
%%       [yosi@aris yosi]$ /usr/local/bin/matlab &
%%       \end{verbatim}

%% \item 少し時間がたつと, Matlab の画面がでてくる.
%% \end{enumerate}

%% \subsubsection{注意事項}

%%   Matlab を起動できるのは同時には一人しか使えません. 下のようなエラ
%% ーが出ると他の人が使っていることになります.
...
%% \subsubsection{Matlab を起動してから}
%% マニュアル等は日本語の Matlab の会社の HP や, 研究室のロッカー兼本棚の一番右端の
%% 上から 3 番目のところ (なんて言えばいいのかな) においてあるので, お読み下さい.
...
%	\item メール用のパスワードの設定
%\begin{verbatim}
%# /usr/sbin/popauth <anyone> パスワード
%\end{verbatim}
%	\item パスワードの変更
%	      各ユーザーには zeus でメール用パスワードを変更してもらいます.
%	      zeus にはユーザーアカウントではログインはできないので, zeus に root でログ
%	      インした後, 以下を実行します.
%\begin{verbatim}
%# su <anyone>       (ユーザー anyone になります)
%$ /usr/sbin/popauth (メールアカウントの変更)
%\end{verbatim}
