...
\begin{itemize}
\item  以下を読んで，筋電位(Electromyography)の性質を理解しておきましょう。\\
  「筋電図の信号品質にかかわる要因とは？」: \url{https://delsys.com/japan/semg-technology}
\item 筋電位が大きいことが，ただちにその筋活動による関節モーメントが大きいことは意味しないことに注意
\end{itemize}
...
\item \textbf{低周波フィルタ or 平滑化} (どちらを用いるかは指示に従うこと)
  ...
  \end{enumerate}
\item \textbf{最大筋電位正規化}: 最大筋電位(フィルタ処理等は同様に行う)により正規化を行う。
\item 必要に応じて時間方向の正規化や加算平均を行う
\end{enumerate}
...
\subsection{データの前処理}
\begin{enumerate}
  \item 数値差分により速度や加速度を求める場合には，軌道データに対して低周波フィルタ(バターワースフィルタ)でノイズ処理を行う。
  \item 周期性のあるデータや，一定時間で行うタスクについては必要に応じて時間方向の正規化や加算平均を行う。
\end{enumerate}
